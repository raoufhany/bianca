            ...
            <tbody>
                @foreach ($rows as $index=>$row)
                    <tr>
                        ...
                        <td class="{{ $row->active == 1 ? 'text-success' : 'text-danger' }}">
                            {{ $row->active == 1 ? trans('common.active') : trans('common.inactive') }}
                        </td>
                        <td>
                            <form method="POST" action="{{ route('admin.category.changeStatus') }}" class="mb-1">
                                @csrf
                                <input name="category_id" value="{{ $row->id }}" hidden>
                                <button type="submit" class="btn btn-primary btn-sm">{{ $row->active == true ? trans('common.Deactivate') : trans('common.Activate')}}</button>
                            </form>
